...
**What it does:**
Finds comparable listings in the dataset within the same category as the thrifted item, calculates average, minimum, and maximum prices, and calls an LLM to generate a style-conscious price assessment explaining if the price is a bargain, fair, or overpriced.

**Input parameters:**
- `item` (dict): The listing dictionary of the selected thrifted piece.

**What it returns:**
A 2-3 sentence markdown string containing the price evaluation and style-conscious reasoning.

**What happens if it fails or returns nothing:**
If the LLM call fails, returns a standard fallback pricing message based on the listing category average.
...
The planning loop is a sequence that orchestrates the tools dynamically:
...
2. **Search Executing & Retry Fallback:**
   - Call `search_listings` using the parsed parameters.
   - If the results are empty:
     - **Fallback 1**: If size was set, retry search with `size=None` (loosen size filter).
     - **Fallback 2**: If still empty (or no size was set but price was), increase `max_price` by 50% and retry search.
     - Log any adjustments made in the session (e.g., `"Loosened size constraint to M"`).
3. **Guard/Branching:** It checks if the search results are still empty. If they are, it sets the session error to a clear message and stops execution early.
4. **Item Selection:** It selects the top-scoring listing as the primary item.
5. **Price Comparison:** Calls `compare_price` on the selected item to evaluate pricing fairness, storing the result in the session.
6. **Outfit Generation:** It calls `suggest_outfit` using the selected item and the user's wardrobe.
7. **Caption Generation:** It calls `create_fit_card` using the outfit suggestion and selected item.
8. **Finalization:** It returns the completed session state dictionary.
...
- `price_assessment` (str): The price fairness evaluation text generated by Tool 4.
- `adjustments` (list[str]): Logs any loosened constraints made during fallback retries.
...
|------|-------------|----------------|
| search_listings | No results match the query | Triggers retry logic by first dropping the size constraint, and then raising the maximum price threshold by 50%. If both fallbacks yield zero results, sets `session["error"]` and exits early. |
...
| compare_price | LLM pricing analysis fails | Returns a friendly fallback pricing message containing the category averages. |
